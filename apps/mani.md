...
約80項目のヒカマニ語録コーパスを手動収集し、システムプロンプトに埋め込んでいます。コウヘイ系・界隈方言など複数の界隈方言に対応するため、モードごとに異なるプロンプトテンプレートを用意しています。